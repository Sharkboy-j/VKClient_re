...
namespace VKClient.Groups.Management.Information
{
  public sealed class InformationViewModel : ViewModelStatefulBase
  {
    ...
    public CommonFieldsViewModel CommonFieldsViewModel { get; set; }//

    public FoundationDateViewModel FoundationDateViewModel { get; set; }

    public AgeLimitsViewModel AgeLimitsViewModel { get; set; }

    public CommunityTypeSelectionViewModel CommunityTypeSelectionViewModel { get; set; }

    public EventOrganizerViewModel EventOrganizerViewModel { get; set; }

    public EventDatesViewModel EventDatesViewModel { get; set; }

    public CommunityPlacementViewModel CommunityPlacementViewModel { get; set; }
    ...
    public void SaveChanges()
    {
      ...
      string str1 = this.CommonFieldsViewModel.Domain != this.CommonFieldsViewModel.CurrentDomain ? this.CommonFieldsViewModel.Domain : null;
      if (str1 == "")
      ...
      Action<BackendResult<int, ResultCode>> callback = (Action<BackendResult<int, ResultCode>>) (result => Execute.ExecuteOnUIThread((Action) (() =>
      {
        ...
        else
        {
          ...
          switch (error != null ? error.error_text : null)
          {
            case null:
      ...
      current.SetCommunityInformation(id1, name, description, id2, subcategory2, site, accessLevel1, domain, ageLimits, foundationDate, eventOrganizerId, phone, email, unixTimestamp1, unixTimestamp2, callback);
    }
  }
}
