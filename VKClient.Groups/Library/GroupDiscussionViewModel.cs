...
namespace VKClient.Groups.Library
{
  public class GroupDiscussionViewModel : ViewModelBase
  {
    ...
    public Visibility NewCommentVisibility
    {
      get
      {
        return !this._canComment ? Visibility.Collapsed : Visibility.Visible;
      }
    }
    ...
    private void EnsurePollIsAdded(CommentsInfo commentsInfo)
    {
      ...
      Func<IVirtualizable, bool> predicate = (Func<IVirtualizable, bool>)(vi => vi is PollItem);
      if (virtualizableItems.FirstOrDefault<IVirtualizable>(predicate) != null || commentsInfo.poll == null || commentsInfo.poll.poll_id == 0L)
        return;
      PollItem pollItem = new PollItem(480.0, new Thickness(0.0), commentsInfo.poll, this._tid);
      MyVirtualizingPanel2 virtualizingPanel2 = this._virtPanel;
      int index = 0;
      List<IVirtualizable> itemsToInsert = new List<IVirtualizable>();
    ...
    private void editCommentCallback(CommentItem commentItem)
    {
      ...
      Navigator.Current.NavigateToNewWallPost(Math.Abs(commentItem.Comment.owner_id), commentItem.Comment.owner_id < 0L, 0, false, false, false);
    }
      ...
      else
      {
        ...
        GroupsService.Current.AddTopicComment(this._gid, this._tid, commentText, attachments.Select<IOutboundAttachment, string>((Func<IOutboundAttachment, string>) (a => a.AttachmentId)).ToList<string>(), (Action<BackendResult<Comment, ResultCode>>) (res =>
        {
          if (res.ResultCode == ResultCode.Succeeded)
            Execute.ExecuteOnUIThread((Action) (() =>
            {
              CommentItem commentItem = this.CreateCommentItem(AppGlobalStateManager.Current.GlobalState.LoggedInUser, res.ResultData, (VKClient.Common.Backend.DataObjects.Group) null);
              MyVirtualizingPanel2 virtualizingPanel2 = this._virtPanel;
