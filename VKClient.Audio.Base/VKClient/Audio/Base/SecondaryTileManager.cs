    ...
    public class SecondaryTileManager
    {
        ...
        public bool TileExistsForConversation(long userOrChatId, bool isChat)
        {
            if (isChat)
                ...
                {
                    if (t.NavigationUri.OriginalString.Contains(string.Format("VKMessenger;component/Views/ConversationPage.xaml?UserOrChatId={0}&IsChat={1}", (object)userOrChatId, (object)isChat)))
                        return t.NavigationUri.OriginalString.Contains("TileLoggedInUserId=" + (object)AppGlobalStateManager.Current.LoggedInUserId);
                    return false;
                })) != null;
            return ShellTile.ActiveTiles.FirstOrDefault<ShellTile>((Func<ShellTile, bool>)(t => t.NavigationUri.OriginalString.Contains(string.Format("VKMessenger;component/Views/ConversationPage.xaml?UserOrChatId={0}&IsChat={1}", (object)userOrChatId, (object)isChat)))) != null;
        }
        ...
        {
            return ShellTile.ActiveTiles.FirstOrDefault<ShellTile>((Func<ShellTile, bool>)(t => t.NavigationUri.OriginalString.Contains(string.Format("/VKClient.Common;component/Profiles/Shared/Views/ProfilePage.xaml?UserOrGroupId={0}", (object)userOrGroupId))));
        }
        ...
        public void UpdateAllExistingTiles(Action<bool> completionCallback)
        {
            ...
            this.ProcessTile(0, ShellTile.ActiveTiles.Where<ShellTile>((Func<ShellTile, bool>)(t =>
            {
        ...
        private void ProcessTile(int ind, List<ShellTile> list, Action<bool> completionCallback)
        {
            ...
            else
            {
                ...
                this.GetSecondaryTileData(userOrGroupId, isGroup, name, 5, (Action<bool, CycleTileData>)((res, resData) =>
                {
                    if (res)
                    {
                        Logger.Instance.Info("Updating secondary tile with new info " + (object)tile.NavigationUri);
                        try
                        {
                        ...
                        Logger.Instance.Error("Failed to process tile update " + (object)tile.NavigationUri);
            ...
            this.GetRemoteUrisFor(Math.Abs(userOrGroupId), isGroup, (Action<bool, List<string>>)((res, resData) =>
            {
                if (res)
                {
                    ...
                    if (resData.Count == 0)
                    {
                        ...
                        int num2 = 1;
                        action(num2 != 0, new CycleTileData()
                        {
                            Title = SecondaryTileManager.FormatNameForTile(name)
                        });
                    }
                    else
                    {
                        ...
                        new Thread((ThreadStart)(() =>
                        {
                            if (!WaitHandle.WaitAll(waitHandles.ToArray(), 15000))
                                completionCallback(false, (CycleTileData)null);
                            else
                                completionCallback(true, new CycleTileData()
                                {
                                    Title = SecondaryTileManager.FormatNameForTile(name),
                                    CycleImages = (IEnumerable<Uri>)imagesToDownloadData.Where<Tuple<int, string>>(/*func1 ?? (func1 = (*/new Func<Tuple<int, string>, bool>(im => im.Item2 != smallPhoto)).Select<Tuple<int, string>, Uri>(/*func2 ?? (func2 = (*/new Func<Tuple<int, string>, Uri>(im => new Uri("isostore:/" + SecondaryTileManager.GetLocalNameFor(im.Item1, "UserOrGroup", userOrGroupId.ToString() + isGroup.ToString()), UriKind.Absolute))).ToList<Uri>(),
                                    SmallBackgroundImage = smallPhoto == null ? null : new Uri("isostore:/" + SecondaryTileManager.GetLocalNameFor(1000, "UserOrGroup", userOrGroupId.ToString() + isGroup.ToString()), UriKind.Absolute)
                                });
                        })).Start();
                    }
                }
        ...
        private void GetRemoteUrisFor(long userOrGroupId, bool isGroup, Action<bool, List<string>> completionCallback)
        {
            PhotosService.Current.GetAllPhotos(userOrGroupId, isGroup, 0, 9, (Action<BackendResult<PhotosListWithCount, ResultCode>>)(res =>
            {
                ...
                {
                    completionCallback(false, (List<string>)null);
                }
                else
                {
                    ...
                    if (res.ResultData.response != null && res.ResultData.response.Count > 0)
                    {
                        ...
                        PhotosService.Current.GetPhotos(userOrGroupId, isGroup, "profile", (List<long>)null, 0L, "", (Action<BackendResult<List<Photo>, ResultCode>>)(result =>
                        {
        ...
        {
            return "Shared/ShellContent/" + ("SecTileData_" + tileType + "_" + objId + "_" + (object)ind);
        }
    }
}
