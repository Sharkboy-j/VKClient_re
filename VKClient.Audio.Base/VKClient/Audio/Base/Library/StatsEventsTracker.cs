    ...
    public class StatsEventsTracker : IHandle<OpenUserEvent>, IHandle, IHandle<OpenGroupEvent>, IHandle<ViewPostEvent>, IHandle<ViewBlockEvent>, IHandle<OpenPostEvent>, IHandle<OpenVideoEvent>, IHandle<VideoPlayEvent>, IHandle<MenuClickEvent>, IHandle<TransitionFromPostEvent>, IHandle<SubscriptionFromPostEvent>, IHandle<HyperlinkClickedEvent>, IHandle<OpenGamesEvent>, IHandle<GamesActionEvent>, IHandle<MarketItemActionEvent>, IHandle<FriendRecommendationShowedEvent>, IHandle<AppActivatedEvent>, IHandle<AdImpressionEvent>, IHandle<AdPixelEvent>, IHandle<ProfileBlockClickEvent>, IHandle<DiscoverActionEvent>, IHandle<MarketContactEvent>, IHandle<BalanceTopupEvent>, IHandle<StickersPurchaseFunnelEvent>, IHandle<GifPlayEvent>
    {
                ...
                user_ids = new List<long>()
        {
          oue.UserId
            ...
            List<string> list3 = enumerable.OfType<ViewBlockEvent>().Select<ViewBlockEvent, string>((Func<ViewBlockEvent, string>)(e => e.ItemType + "|" + (object)e.Position)).ToList<string>();
            if (list3.Count > 0)
            {
            ...
            Func<TransitionFromPostEvent, string> keySelector1 = (Func<TransitionFromPostEvent, string>)(tps => tps.post_id);
            Dictionary<string, string> dictionary = source2.ToDictionary<TransitionFromPostEvent, string, string>(keySelector1, (Func<TransitionFromPostEvent, string>)(tps => tps.parent_id));
            if (list6.Count > 0)
            {
            ...
            appEventBaseList.AddRange((IEnumerable<AppEventBase>)source3.Distinct<AppEventVideoPlay, string>((Func<AppEventVideoPlay, string>)(e => e.video_id)));
        ...
        public void Handle(HyperlinkClickedEvent message)
        {
            if (string.IsNullOrEmpty(message.HyperlinkOwnerId))
                return;
            this.HandleEvent((object)message, false, true);
        }
        ...
        private void LoadUris(List<string> list)
        {
            foreach (string str in list)
            {
                ...
                if (!this._loadedUris.ContainsKey(uri))
                    JsonWebRequest.Download(uri, 0L, 0L, (Action<HttpStatusCode, long, byte[]>)((statusCode, b, c) =>
                    {
        ...
        {
            this.HandleEvent((object)eventBase, eventBase.ShouldSendImmediately, false);
        }
