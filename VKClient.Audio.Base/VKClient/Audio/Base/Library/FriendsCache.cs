...
namespace VKClient.Audio.Base.Library
{
    public class FriendsCache
    {
        ...
        private string Key
        {
            get
            {
                return "FriendsOf_" + (object)AppGlobalStateManager.Current.LoggedInUserId;
            }
        }
        ...
        public async void SetFriends(List<User> friends, FriendRequests requests)
        {
            ...
            EventAggregator.Current.Publish((object)new FriendsCacheUpdated());
            await CacheManager.TrySerializeAsync((IBinarySerializable)savedContacts, this.Key, false, CacheManager.DataType.CachedData);//
        }
    }
}
