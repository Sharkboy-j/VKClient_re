...
namespace VKClient.Common.Framework
{
    ...
    {
        public static Action<Action> DefaultPublicationThreadMarshaller = new Action<Action>(action => { Execute.ExecuteOnUIThread(action); });
        ...
        public virtual void Unsubscribe(object instance)
        {
            lock (this.handlers)
            {
                EventAggregator.Handler local_3 = this.handlers.FirstOrDefault<EventAggregator.Handler>((Func<EventAggregator.Handler, bool>)(x => x.Matches(instance)));
                if (local_3 == null)
                    return;
                this.handlers.Remove(local_3);
            }
        }
        ...
        protected class Handler
        {
            ...
            public Handler(object handler)
            {
                ...
                {
                    Type index = type.GetGenericArguments()[0];
                    string name = "Handle";
                    MethodInfo method = type.GetMethod(name);
                    this.supportedHandlers[index] = method;
                }
            }
