...
namespace VKClient.Common.Library
{
    public static class AudioTrackHelper
    {
        ...
        public static AudioTrack CreateTrack(AudioObj audio)
        {
            if (audio == null)
                return null;
            //string uriString = "ms-appx:///Resources/New/chat_sound.wav";
            string uriString = string.IsNullOrEmpty(audio.url) ? "http://vk.com/404.mp3" : audio.url;
            ...
            AudioTrack temp = new AudioTrack(new Uri(uriString, UriKind.RelativeOrAbsolute), audio.title ?? "NOTITLE", audio.artist ?? "NOARTIST", "", null, audio.UniqueId + "|" + audio.duration, EnabledPlayerControls.All);
            return temp;
        }
            ...
            try
            {
                ...
                if (track != null && track.Source != null)
                {
                    ...
                    if (track.Source.OriginalString.StartsWith("http") && track.GetTagOwnerId() == AppGlobalStateManager.Current.LoggedInUserId && AppGlobalStateManager.Current.GlobalState.IsMusicCachingEnabled)
                    {
                        ...
                        Uri source2 = track.Source;
                        string tag = track.Tag;
                        playedFilesInfo.Add(source2, tag);
                    }
        ...
        public static void BroadcastTrackIfNeeded(BackgroundAudioPlayer player, List<AudioObj> playlist = null, PlaybackSettings settings = null, bool allowCache = false, bool bypassChecks = false)
        {
            try
            {
                if (!bypassChecks && (player.PlayerState != PlayState.Playing || (DateTime.Now - AudioTrackHelper._lastTimeBroadcastAttempt).TotalSeconds < 3.0))
                    return;
