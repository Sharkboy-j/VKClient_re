...
namespace VKClient.Common.Backend
{
    public class MapsService
    {
        ...
        public static MapsService Current { get; set; }
        ...
        public Uri GetMapUri(double latitude, double longitude, int zoomLevel, int width, double heightDivisionFactor)
        {
            ...
            return new Uri(string.Format(MapsService._mapUriFormat, (object)this.CoordinatesToString(latitude, longitude), (object)zoomLevel, (object)num5, (object)num6, (object)num3, (object)CultureInfo.CurrentCulture));
        }

        public void ReverseGeocodeToAddress(double latitude, double longitude, Action<BackendResult<string, ResultCode>> callback)
        {
            JsonWebRequest.SendHTTPRequestAsync(string.Format(MapsService._reverseGeocodeUriFormat, (object)this.CoordinatesToString(latitude, longitude), (object)CultureInfo.CurrentCulture), (Action<JsonResponseData>)(data =>
            {
                if (!data.IsSucceeded)
                {
        ...
        {
            return string.Format("{0},{1}", (object)latitude.ToString((IFormatProvider)CultureInfo.InvariantCulture), (object)longitude.ToString((IFormatProvider)CultureInfo.InvariantCulture));
        }
