...
namespace VKClient.Common.Backend
{
    public static class VKRequestsDispatcher
    {
        ...
        public static bool HaveToken
        {
            get
            {
                AutorizationData autorizationData = VKRequestsDispatcher._autorizationData;
                return !string.IsNullOrEmpty(autorizationData != null ? autorizationData.access_token : null);
            }
        }
        ...
        public static string Token
        {
            get
            {
                if (VKRequestsDispatcher._autorizationData == null)
                    return "";
                return VKRequestsDispatcher._autorizationData.access_token;
            }
        }
        ...
        public static void DispatchLoginRequest(string login, string password, string code, bool forceSms, Action<BackendResult<AutorizationData, ResultCode>> callback, CancellationToken? cancellationToken = null)
        {
            ...
            Dictionary<string, string> parameters = dictionary1;
            if (!string.IsNullOrEmpty(code))
                parameters["code"] = code;
            else if (forceSms)
                parameters["force_sms"] = "1";
            parameters["scope"] = "notify, friends, photos, audio, video, docs, notes, pages, status, wall, groups, messages, notifications, stats, market";
            if (VKRequestsDispatcher.USE_HTTP)
            {
                Dictionary<string, string> dictionary2 = parameters;
                dictionary2["scope"] = dictionary2["scope"] + ", nohttps";
            }
            VKRequestsDispatcher.DoDispatch<AutorizationData>(VKRequestsDispatcher.LoginUri, "", parameters, callback, new Func<string, AutorizationData>(JsonConvert.DeserializeObject<AutorizationData>), false, true, cancellationToken);
        }
        ...
        public static void Execute<T>(string code, Action<BackendResult<T, ResultCode>> callback, Func<string, T> customDesFunc = null, bool lowPriority = false, bool pageDataRequest = true, CancellationToken? cancellationToken = null) where T : class
        {
            ...
            parameters["code"] = code;
            VKRequestsDispatcher.DispatchRequestToVK<T>("execute", parameters, callback, customDesFunc, lowPriority, pageDataRequest, cancellationToken);
        }

        public static void DispatchRequestToVK<R>(string methodName, Dictionary<string, string> parameters, Action<BackendResult<R, ResultCode>> callback, Func<string, R> customDeserializationFunc = null, bool lowPriority = false, bool pageDataRequest = true, CancellationToken? cancellationToken = null)
        {
            VKRequestsDispatcher.DoDispatch<R>(string.Format(VKRequestsDispatcher.RequestUriFrm, (object)methodName), methodName, parameters, callback, customDeserializationFunc, lowPriority, pageDataRequest, cancellationToken);
        }

        private static void DoDispatch<R>(string baseUrl, string methodName, Dictionary<string, string> parameters, Action<BackendResult<R, ResultCode>> callback, Func<string, R> customDeserializationFunc = null, bool lowPriority = false, bool pageDataRequest = true, CancellationToken? cancellationToken = null)
        {
            ...
            JsonWebRequest.SendHTTPRequestAsync(!VKRequestsDispatcher.USE_HTTP ? "https://" + baseUrl : "http://" + baseUrl, parameters, (Action<JsonResponseData>)(jsonResp =>
            {
                ...
                if (jsonResp.IsSucceeded)
                {
                    ...
                    {
                        AutorizationData autorizationData3 = VKRequestsDispatcher._autorizationData;
                        if (!string.IsNullOrEmpty(autorizationData3 != null ? autorizationData3.access_token : null))
                        {
                    ...
                    if (backendResult.ResultCode == ResultCode.CaptchaRequired)
                        ...
                        }, (Action<CaptchaUserResponse>)(captchaResp =>
                        {
                            if (!captchaResp.IsCancelled)
                            {
                                ...
                                VKRequestsDispatcher.DoDispatch<R>(baseUrl, methodName, parameters1, callback, customDeserializationFunc, lowPriority, pageDataRequest, cancellationToken);
                            }
                            else
                    ...
                    else if (backendResult.ResultCode == ResultCode.ConfirmationRequired)
                    {
                        if (!VKRequestsDispatcher.GetIsResponseCancelled(cancellationToken))
                        {
                            ...
                            {
                                string confirmationText = resultFromJson.confirmation_text;
                                confirmationHandler.Confim(confirmationText, (Action<bool>)(confirmed =>
                                {
                                    if (confirmed)
                                    {
                                        parameters["confirm"] = "1";
                                        VKRequestsDispatcher.DoDispatch<R>(baseUrl, methodName, parameters, callback, customDeserializationFunc, lowPriority, pageDataRequest, cancellationToken);
                                    }
                                    else
                    ...
                    else if (backendResult.ResultCode == ResultCode.NotEnoughMoney)
                    {
                        if (!VKRequestsDispatcher.GetIsResponseCancelled(cancellationToken))
                        {
                            ...
                            Action action1 = (Action)(() => VKRequestsDispatcher.DoDispatch<R>(baseUrl, methodName, parameters, callback, customDeserializationFunc, lowPriority, pageDataRequest, cancellationToken));
                    ...
                    else if (backendResult.ResultCode == ResultCode.Succeeded)
                    {
                        try
                        {
                            R r = customDeserializationFunc == null ? (typeof(R) == typeof(VKClient.Common.Backend.DataObjects.ResponseWithId) ? JsonConvert.DeserializeObject<R>(jsonResp.JsonString) : JsonConvert.DeserializeObject<VKRequestsDispatcher.GenericRoot<R>>(jsonResp.JsonString).response) : customDeserializationFunc(jsonResp.JsonString);
                ...
                if (backendResult.ResultCode == ResultCode.CaptchaRequired || backendResult.ResultCode == ResultCode.ValidationRequired || (backendResult.ResultCode == ResultCode.ConfirmationRequired || backendResult.ResultCode == ResultCode.NotEnoughMoney))
                    return;
                VKRequestsDispatcher.InvokeCallback((Action)(() => callback(backendResult)), cancellationToken);
        ...
        private static void ProcessValidationResponse<R>(ValidationUserResponse valResp, string baseUrl, string methodName, Dictionary<string, string> parameters, Action<BackendResult<R, ResultCode>> callback, Func<string, R> customDeserializationFunc = null, bool lowPriority = false, bool pageDataRequest = true, CancellationToken? cancellationToken = null)
        {
            if (valResp.IsSucceeded)
            {
                ...
                {
                    string str = string.Format("{{\"access_token\":\"{0}\",\"user_id\":{1} }} ", (object)valResp.access_token, (object)valResp.user_id);
                    ...
                    if (jtoken != null)
                    {
                        ...
                        if (errorList != null)
                        {
                            if (errorList.Count > 0)
                                return VKRequestsDispatcher.GetResultFromJson(string.Format("{{\"error\":{0}}}", (object)JsonConvert.SerializeObject((object)errorList[0])));
                        }
                    }
                }
            }
        ...
        private static VKRequestsDispatcher.ResultData GetResultFromJson(JObject jObject)
        {
            ...
            try
            {
                ...
                if (jtoken1 != null)
                {
                    VKRequestsDispatcher.Error @object;
                    if (jtoken1.HasValues)
                    {
                        JToken jtoken2 = jtoken1[(object)"error_code"];
                        if (jtoken2 != null)
                        {
                            int num = (int)jtoken2;
                            resultData.ResultCode = (ResultCode)num;
                        }
                        @object = jtoken1.ToObject<VKRequestsDispatcher.Error>();
                    }
                    else
                    {
                        string @string = jtoken1.ToString();
                        if (!(@string == "need_validation"))
                        {
                            if (!(@string == "need_captcha"))
                            {
                                if (!(@string == "invalid_client"))
                                {
                                    if (!(@string == "invalid_request"))
                                    {
                                        if (@string == "need_confirmation")
                            ...
                            resultData.ResultCode = ResultCode.ValidationRequired;
                        @object = jObject.ToObject<VKRequestsDispatcher.Error>();
                    }
                    resultData.error = @object;
                    resultData.captcha_sid = @object.captcha_sid;
                    resultData.captcha_img = @object.captcha_img;
                    resultData.redirect_uri = @object.redirect_uri;
                    resultData.validation_type = @object.validation_type;
                    resultData.validation_sid = @object.validation_sid;
                    resultData.phone_mask = @object.phone_mask;
                    resultData.confirmation_text = @object.confirmation_text;
                    return resultData;
                }
                ...
                if (jtoken3 != null)
                {
                    List<VKRequestsDispatcher.Error> @object = jtoken3.ToObject<List<VKRequestsDispatcher.Error>>();
                    if (@object != null)
                    {
                        if (@object.Count > 0)
                        {
                            VKRequestsDispatcher.Error error = @object[0];
                            switch ((ResultCode)error.error_code)
                            {
                                case ResultCode.CaptchaRequired:
                                case ResultCode.ValidationRequired:
                                    return VKRequestsDispatcher.GetResultFromJson(JObject.FromObject((object)new VKRequestsDispatcher.RootObjectError()
                                    {
                                        error = error
                                    }));
                            }
            ...
            {
                resultData.ResultCode = ResultCode.UnknownError;
                Logger.Instance.Error("Failed to parse error code, jsonStr =" + (object)jObject, ex);
            }
            return resultData;
        }
