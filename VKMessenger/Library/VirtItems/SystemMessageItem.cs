  ...
  public class SystemMessageItem : VirtualizableItemBase
  {
    ...
    private void CreateOrUpdateLayout()
    {
      ...
      if (this.IsUnreadItem)
      {
        ...
        top = this._unreadItem.FixedHeight;
      }
      else
      {
        if (this._newsTextItem == null)
        {
          this._newsTextItem = new NewsTextItem(this._verticalWidth, new Thickness(), this.GetText(), false, null, 0.0, (FontFamily) null, 0.0, (Brush) (Application.Current.Resources["PhoneVKSubtleBrush"] as SolidColorBrush), this._isHorizontal, this._horizontalWidth, HorizontalAlignment.Center, "", TextAlignment.Center, true);
        ...
        top = this._newsTextItem.FixedHeight + 12.0;
        if (this._mvm.Message.attachments != null && this._mvm.Message.attachments.Count > 0)
        {
          if (this._attachmentsItem == null)
          {
            this._attachmentsItem = new AttachmentsItem(this.ATTACHMENT_WIDTH, new Thickness((this.Width - this.ATTACHMENT_WIDTH) / 2.0, top, 0.0, 0.0), this._mvm.Message.attachments, (Geo) null, "", false, false, true, this._isHorizontal, this.ATTACHMENT_WIDTH, false, false, "");
            this.VirtualizableChildren.Add((IVirtualizable) this._attachmentsItem);
          }
          else
          {
            this._attachmentsItem.IsHorizontal = this._isHorizontal;
            this._attachmentsItem.Margin = new Thickness((this.Width - this.ATTACHMENT_WIDTH) / 2.0, top, 0.0, 0.0);
          }
          top += this._attachmentsItem.FixedHeight;
        }
      }
      this._fixedHeight = top;
    }
