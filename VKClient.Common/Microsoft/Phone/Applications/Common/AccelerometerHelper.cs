...
namespace Microsoft.Phone.Applications.Common
{
    public sealed class AccelerometerHelper : IDisposable
    {
        ...
        private static Simple3DVector AccelerometerCalibrationPersisted
        {
            ...
            set
            {
                if (!(ApplicationSettingHelper.AddOrUpdateValue("AccelerometerCalibrationX", (object)value.X) | ApplicationSettingHelper.AddOrUpdateValue("AccelerometerCalibrationY", (object)value.Y)))
                    return;
        ...
        private AccelerometerHelper()
        {
            ...
            this.NoAccelerometer = this._sensor == null || this._sensor.State == SensorState.NotSupported;
            this._sensor = (Accelerometer)null;
            this._sampleIndex = 0;
        ...
        public bool CanCalibrate(bool xAxis, bool yAxis)
        {
            ...
            lock (this._sampleBuffer)
            {
                if (this.IsDeviceStable)
                {
                    double local_3 = 0.0;
                    if (xAxis)
                        local_3 += this._averageAcceleration.X * this._averageAcceleration.X;
                    if (yAxis)
                        local_3 += this._averageAcceleration.Y * this._averageAcceleration.Y;
                    if (Math.Sqrt(local_3) <= AccelerometerHelper._maximumCalibrationOffset)
                        flag = true;
                }
            }
            return flag;
        ...
        private void sensor_ReadingChanged(object sender, AccelerometerReadingEventArgs e)
        {
            ...
            lock (this._sampleBuffer)
            {
                if (!this._initialized)
                {
                    ...
                    for (int local_8 = 0; local_8 < 25; ++local_8)
                        this._sampleBuffer[local_8] = this._averageAcceleration;
                ...
                if (this._sampleIndex >= 25)
                    this._sampleIndex = 0;
                Simple3DVector local_6 = simple3Dvector3;
                this._sampleSum = this._sampleSum + local_6;
                this._sampleSum = this._sampleSum - this._sampleBuffer[this._sampleIndex];
                this._sampleBuffer[this._sampleIndex] = local_6;
                simple3Dvector4 = this._sampleSum / 25.0;
                this._averageAcceleration = simple3Dvector4;
                Simple3DVector local_7 = simple3Dvector4 - simple3Dvector3;
                if (Math.Abs(local_7.X) > AccelerometerHelper._maximumStabilityDeltaOffset || Math.Abs(local_7.Y) > AccelerometerHelper._maximumStabilityDeltaOffset || Math.Abs(local_7.Z) > AccelerometerHelper._maximumStabilityDeltaOffset)
            ...
            if (this.ReadingChanged == null)
                return;
            this.ReadingChanged((object)this, new AccelerometerHelperReadingEventArgs()
            {
