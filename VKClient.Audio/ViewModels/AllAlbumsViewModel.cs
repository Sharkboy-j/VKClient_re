...
namespace VKClient.Audio.ViewModels
{
  ...
  {
    public static readonly long SAVED_ALBUM_ID = -99;
    ...
    public GenericCollectionViewModel<ListResponse<AudioAlbum>, AudioAlbumHeader> AllAlbums
    {
      ...
      set
      {
        ...
        this.NotifyPropertyChanged<GenericCollectionViewModel<ListResponse<AudioAlbum>, AudioAlbumHeader>>((Expression<Func<GenericCollectionViewModel<ListResponse<AudioAlbum>, AudioAlbumHeader>>>) (() => this.AllAlbums));
      }
    }
    ...
    public Func<ListResponse<AudioAlbum>, ListWithCount<AudioAlbumHeader>> ConverterFunc
    {
      get
      {
        return (Func<ListResponse<AudioAlbum>, ListWithCount<AudioAlbumHeader>>) (input => new ListWithCount<AudioAlbumHeader>()
        {
          ...
          List = input.Data.Select<AudioAlbum, AudioAlbumHeader>((Func<AudioAlbum, AudioAlbumHeader>) (a => new AudioAlbumHeader(a))).ToList<AudioAlbumHeader>()
        });
      }
    }
      ...
      AudioService.Instance.GetUserAlbums((Action<BackendResult<ListResponse<AudioAlbum>, ResultCode>>) (res =>
      {
        if (res.ResultCode == ResultCode.Succeeded)
        {
          if (!this._isPickMode && offset == 0 && !this.IsGroup)
          {
            ...
            {
              title = CommonResources.Audio_Cached,
            ...
            ++res.ResultData.TotalCount;
            res.ResultData.Data.Insert(2, new AudioAlbum()
            {
          ...
          {
            --res.ResultData.TotalCount;
            AudioAlbum audioAlbum = res.ResultData.Data.FirstOrDefault<AudioAlbum>((Func<AudioAlbum, bool>) (a => a.album_id == this._excludeAlbumId));
            if (audioAlbum != null)
            {
    ...
    internal void CreateEditAlbum(AudioAlbum album)
    {
      if (album.album_id == 0L)
      {
        AudioService.Instance.CreateAlbum(album.title, (Action<BackendResult<AudioAlbum, ResultCode>>) (res =>
        {
          ...
          EventAggregator.Current.Publish((object) new AudioAlbumCreatedEditedDeleted()
          {
    ...
    public void Handle(AudioAlbumCreatedEditedDeleted message)
    {
      ...
      if (created.HasValue)
      {
        AudioAlbumHeader audioAlbumHeader = this._allAlbums.Collection.FirstOrDefault<AudioAlbumHeader>((Func<AudioAlbumHeader, bool>) (a => a.Album.album_id == message.Album.album_id));
