...
namespace XamlAnimatedGif.Decoding
{
    internal static class GifHelpers
    {
        ...
        {
            MemoryStream ms = discard ? (MemoryStream)null : new MemoryStream();
            byte[] numArray;
            using (ms)
            {
                int len;
                ...
                numArray = memoryStream1 != null ? memoryStream1.ToArray() : null;
            }
            return numArray;
        }

        public static async Task CopyDataBlocksToStreamAsync(Stream sourceStream, Stream targetStream, CancellationToken cancellationToken)
        {
            while (true)
            {
        ...
        {
            return (Exception)new InvalidBlockSizeException(string.Format("Invalid block size for {0}. Expected {1}, but was {2}", (object)blockName, (object)expectedBlockSize, (object)actualBlockSize));
        }
